''' LC unit class file w functions & differential equations from Alvarez & al 2002 paper- formulated for Euler-Murayama method
 15/4/2020 edits for opto pair simulations, this reciever unit takes g_i from alphatrace produced by stimulated neuron,
  to model Ray Perrins data.
...
class LC_unit(object):
    ...
    def update(self, i, dt, ttx_on, carb_on, g_i):
        ...
        # Differentials
        wnoise = np.random.normal(loc = 0.0, scale = 0.015)
        #wnoise = 0
        ...
        self.ca_conc += dt*(-0.002*2*(self.v_s - self.v_ca)/(1+np.exp(-(self.v_s+25)/2.5)) - self.ca_conc/160)
        self.y += dt*(0.75 * (y_inf_v - self.y)/tau_y_v)
        return  self.v_s, self.v_d, self.ca_conc, i_a2
    ...
    def alpha_func(self,li_tau):
        ...
        while len(results) <= li_tau * 100000:
                # exponential decay function
            g_i =  ((t-t_i)/(li_tau*10)) * np.exp(-(t-t_i)/(li_tau*10))
            results.append(g_i)
            #print(g_i)
            t += 1
        return results
